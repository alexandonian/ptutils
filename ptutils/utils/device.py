...
from shell import execute_process
...
class DeviceCollection(object):
    ...
    def new_query():
        ...
        for line in smi_output.split('\n'):
            ...
            g = Device({col_name: col_value.strip()for (col_name, col_value)
                        in zip(gpu_query_columns, query_results)})
            gpu_list.append(g)

        return DeviceCollection(gpu_list)
...
def self_test():
    gpu_stats = DeviceCollection.new_query()
    print('# of GPUS:', len(gpu_stats))
    for g in gpu_stats:
        print(g)

    process_entries = DeviceCollection.running_processes()
    print('---Entries---')
...
def new_query():
    '''
    Obtain a new DeviceCollection instance by querying nvidia-smi
    to get the list of GPUs and running process information.
    '''
    return DeviceCollection.new_query()
